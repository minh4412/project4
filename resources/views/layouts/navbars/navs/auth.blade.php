    ...
    <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse" id="navigation">
            <ul class="navbar-nav ml-auto">
                ...
                @if (Session('role') == 0)
                <li class="dropdown nav-item" onclick="location.href='{{ url('/mycart') }}'">
                    <a href="{{ url('/mycart') }}" class="dropdown-toggle nav-link" data-toggle="dropdown">
                        <div class="notification d-none d-lg-block d-xl-block
                        @if(Session('cart') == null) 
                            d_hidden
                        @endif"></div>
                        <i class="tim-icons icon-cart"></i>
                        <p class="d-lg-none"> {{ __('Cart') }} </p>
                    </a>
                </li>
                @endif
                <li class="dropdown nav-item">
                    ...
                    <ul class="dropdown-menu dropdown-navbar">
                        <li class="nav-link">
                            <a href="{{ route('profile.index') }}" class="nav-item dropdown-item">{{ __('Profile') }}</a>
                        </li>
                        <li class="nav-link">
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
	            <form action="">
                    <input type="text" class="form-control" id="inlineFormInputGroup" placeholder="{{ __('SEARCH') }}" name="search" value="{{ isset($search) ? $search : "" }}">
                </form>
