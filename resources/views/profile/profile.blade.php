    ...
    <div class="row">
        <div class="col-md-8">
            <div class="card">
                ...
                <form method="post" action="{{ route('profile.update', $userinfo->user_id) }}" autocomplete="off">
                    <div class="card-body">
                            @csrf
                            @method('put')
                            ...
                            <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                                <label>{{ _('Name') }}</label>
                                <input type="text" name="name" class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" placeholder="{{ _('Name') }}" value="{{ $userinfo->name }}">
                                @include('alerts.feedback', ['field' => 'name'])
                            </div>

                            <div class="form-group{{ $errors->has('email') ? ' has-danger' : '' }}">
                                <label>{{ _('Email address') }}</label>
                                <input type="email" name="email" class="form-control{{ $errors->has('email') ? ' is-invalid' : '' }}" placeholder="{{ _('Email address') }}" value="{{ $userinfo->email }}">
